...
SELECT name, monthlymaintenance,
	CASE WHEN monthlymaintenance > 100 THEN 'expensive'
	     WHEN monthlymaintenance < 100 THEN 'cheap' END AS cost_level
FROM Facilities;	
...
SELECT DISTINCT Facilities.name AS court_name, CONCAT_WS('  ',Members.surname,Members.firstname) AS member_name
FROM Members
INNER JOIN Bookings 
...
SELECT f.name AS Facility,CONCAT_ws('  ',m.firstname, m.surname) AS Member,
  (CASE WHEN m.memid = 0 THEN (f.guestcost * b.slots)
   ELSE (f.membercost * b.slots) END) AS Cost,b.starttime
FROM Bookings as b
LEFT JOIN Members as m
ON m.memid = b.memid
LEFT JOIN Facilities as f
